...
if(!empty($sktoken) &&!empty($res_id)){

  try{
    $jwt_decoded = JWT::decode($sktoken, $secret_key, array('HS256'));
  } catch(Exception $e){
    $jwt_error = $e->getCode();
    $jwt_error_result = "set_owner_schedule_cancel : ".$e->getMessage();
  }
 ...
 //userinfo 테이블에 있는 유저 정보를 추출하기 위한 query문
  $query = "UPDATE reservation SET state =3 where reservation_id = $res_id and owner_id =$user_id" ; //list
...
if(empty($result_array['err_result'])){
  ...
  $result_array['err_result']=get_err_result("set_owner_schedule_cancel",$result_array['err']);
}
