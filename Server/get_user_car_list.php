...
if(!empty($sktoken)){
  ...
  try{
    ...
    $jwt_error = $e->getCode();
    $jwt_error_result = "get_user_car_list : ".$e->getMessage();
  }
 ...
 //userinfo 테이블에 있는 유저 정보를 추출하기 위한 query문
  $query = "SELECT carinfo_id,model,model_year,carinfo_img_t FROM carinfo where userinfo_id='$user_id'" ; //list
...
if(empty($result_array['err_result'])){
  ...
  $result_array['err_result']=get_err_result("get_user_car_list",$result_array['err']);
}
