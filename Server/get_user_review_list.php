...
if(!empty($sktoken) &&(!empty($type)||$type==0)&&(!empty($offset)||$offset==0)&&!empty($row_cnt)){
  $jwt_error='0';
...
//userinfo 테이블에 있는 유저 정보를 추출하기 위한 query문
    $query ="SELECT writer_id,writer_name,content,date,score FROM userreview where owner_id=$user_id AND type=$type ORDER BY date DESC LIMIT $offset,$row_cnt";
