...
if(!empty($sktoken) &&!empty($target_id) &&(!empty($type)||$type==0)&&(!empty($offset)||$offset==0)&&!empty($row_cnt)){
  $jwt_error='0';
  ...
  try{
    ...
    $jwt_error = $e->getCode();
    $jwt_error_result = "get_user_review_list : ".$e->getMessage();
  }
...
//userinfo 테이블에 있는 유저 정보를 추출하기 위한 query문
    $query ="SELECT writer_id,writer_name,content,date,score FROM userreview where owner_id=$target_id AND type=$type ORDER BY date DESC LIMIT $offset,$row_cnt";
...
if(empty($result_array['err_result'])){
  ...
  $result_array['err_result']=get_err_result("get_user_review_list",$result_array['err']);
}
