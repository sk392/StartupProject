...
$result_array = array();
if(!is_null($sktoken)&&!is_null($type)){
 ...
 //userinfo 테이블에 있는 유저 정보를 추출하기 위한 query문
    $query = "SELECT * FROM userinfo where userinfo_id='$user_id'" ;
  //결과 값을 받기 위한 배열
  ...
  //차량 이용정보 
        $query = "SELECT mileage, isaccident FROM reservation where userinfo_id ='$user_id' AND state =2";
        if($result = mysql_query($query)){
          $result_array['err']='0';
          $accident_cnt=0;
          $mileage=0;
